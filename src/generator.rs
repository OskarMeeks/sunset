...
// ─────────────────────────────────────────────
//  indicators::AsCascadeBar impl for StockData
//  Lives here (not in the binary) to satisfy Rust's orphan rule:
//  StockData is defined in this crate, so this is the right place.
// ─────────────────────────────────────────────

impl crate::indicators::AsCascadeBar for StockData {
    fn to_bar(&self) -> crate::indicators::Bar {
        crate::indicators::Bar {
            open:   self.open,
            high:   self.high,
            low:    self.low,
            close:  self.close,
